...
Puppet::Type.type(:package).provide :gem, :parent => Puppet::Provider::Package do
  ...
  if Puppet.features.microsoft_windows?
    ...
    # One of those paths includes Puppet's RUBY_DIR.
    # That results in the which() method in self.command(name) in
    # lib/puppet/provider.rb always returning Puppet's ruby 'gem' command.
